...
const LoginPage = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validate() === true) login(formData);
  };
    ...
    <div className="flex min-h-screen items-center justify-center px-4 pt-16">
      ...
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-zinc-500">Sign in to continue to ChatApp</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            ...
            {!isLoggingIn && <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />}
          </button>
        </form>

        <p className="mt-8 text-center text-sm text-zinc-600">
          Don&apos;t have an account?{' '}
          <Link to="/signup" className="font-medium text-zinc-300 transition-colors hover:text-white">
            Create one
          </Link>
        </p>
      </div>
